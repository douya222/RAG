...
def prepare_data(path, size):
    ...
    for file in os.listdir(path):
        with open(path + '/' + file, 'r', encoding='utf-8') as f:
            content=f.read()
            c_p[content]=file
            print(f'key:{content},value:{c_p[content]}')
...
def get_info(question):
    ...
    for doc in rets.output:
        ...
        print(c_p[doc.fields['question']])
        with open('data/' + c_p[doc.fields['question']], 'r', encoding='utf-8') as f:
            print('ok2')
            inf+=f.read()
            inf+='\n'
            print('ok3')
...
if __name__ == '__main__':
    ...
    response=llm('藜怎么防治虫害？')
    print(response)
    print(answer_question('藜怎么防治虫害？'))
